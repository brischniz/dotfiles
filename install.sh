...
if ! sudo grep -q "%wheel		ALL=(ALL) NOPASSWD: ALL #atomantic/dotfiles" "/etc/sudoers"; then
  ...
  if [[ $response =~ (yes|y|Y) ]];then
      ...
      echo '%wheel		ALL=(ALL) NOPASSWD: ALL #brischniz/dotfiles' | sudo tee -a /etc/sudoers > /dev/null
...
running "Top left screen corner → Show App Windows"
...
bot "Woot! All done. Kill this terminal and launch again"
